...
export default function OrdersPage() {
  ...
  useEffect(() => {
    axios.get(`/api/orders?status=${activeStatus}&limit=20`)
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {orders.map((order: any) => (
              <Link key={order.id} to={`/order-success/${order.id}`}
                className="block bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <span className="font-mono text-sm font-medium text-gray-600">#{order.id.slice(0, 8).toUpperCase()}</span>
                    <span className="ml-3 text-xs text-gray-400">{new Date(order.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColors[order.status] || 'bg-gray-100 text-gray-600'}`}>{order.status}</span>
